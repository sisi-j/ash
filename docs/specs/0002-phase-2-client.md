...
- **Three things are unverified and must not be assumed while ticketing.** ~~Whether Fabric's client game tests run on a GPU-less Windows CI runner — ADR-0016 says outright that if they do not, ADR-0017 should be reopened rather than worked around.~~ *Answered 2026-09-19, while implementing #22: they run on a GPU-less **Linux** runner, on both targets, and ADR-0017 is not reopened. On Windows they still do not run and are not expected to — `xvfb-run` is a Linux thing and Loom wraps a client run in it nowhere else. See ADR-0016.* Legacy Fabric API's pinnable version, where three sources give three different answers and it must be resolved empirically. And the vanilla class and method names toggle sprint's mixin needs on each target, which need a decompiled-source pass before any ticket can specify them.
