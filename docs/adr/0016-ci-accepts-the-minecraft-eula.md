...
- **Both tiers now go into a world, and getting there took two findings.** ash's HUD draws only in a world, and the player - whose movement tick is where toggle sprint's mixin lands - only exists in one, so a test that stops at the title screen passes with all of ash's in-game code unexercised. Added while implementing #23; both failed on their first run, for unrelated reasons.

  **1.8.9 opens its pause menu the moment a world is up**, because under Xvfb the window never has focus. The smoke test waited three minutes for a screen that would never close. It now turns `pauseOnLostFocus` off first, and is in a world in about fourteen seconds.

  **1.21.11's singleplayer world can never finish loading under Fabric's test framework on this runner.** It sat at `Preparing spawn area: 16%` until the framework's minute ran out, with the world-generation workers idle. The chain, read from the bytecode: since 1.21.9 a joining player's spawn is prepared as a configuration task, before they are in the player list; an integrated server pauses whenever that list is empty (`IntegratedServer.tickServer`), and a paused one ticks its connections but not its levels; so the spawn chunks can only load in idle time, and `MinecraftServer.pollTaskInternal` polls chunks there only if the server is sprinting, running all tasks, or `haveTime()` - ahead of schedule. Under the framework's lockstep ticking it was 40 ticks behind. A thread dump settled it: server, render and test threads all at the framework's phaser, workers with nothing to do. So the modern test joins a **dedicated server** instead, which does not pause when empty; it is up in half a second. An earlier guess, that software rendering was starving the server of CPU, was wrong, and the frame-rate cap it motivated is gone.

  The two in-world screenshots CI keeps show the FPS readout top-left and the marker bottom-left on both targets, the same text in the same place.
