...
- Toggle sprint is the feature that proves the rule has teeth, which is why it is in Phase 2 alongside something as simple as an FPS readout. Its binding helper lives in a different package on each target, over a different key type, and its latch is exactly the kind of state machine the shared module exists to hold.
